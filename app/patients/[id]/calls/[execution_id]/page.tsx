...
export default function CallDetailPage() {
  ...
  async function fetchPatientName(pid?: string | null) {
    ...
    try {
      ...
      const name = data?.name || data?.patient?.name || data?.data?.name || null;
      if (name) setPatientName(name);
    } catch {
      // ignore; fallback to default "Patient"
    }
  }
  ...
  async function fetchDetail() {
    ...
    setErrorMsg("");
    try {
      const res = await fetch(`${API_BASE}/call/${execution_id}/detail`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  if (errorMsg || !detail) {
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-4xl mx-auto">
          <button
            onClick={() => router.push(`/patients/${id}/calls`)}  // ✅ correct template literal
...
onClick={() => router.push(`/patients/${id}/calls`)}  // ✅ correct template literal
  ...
  <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 mb-6">
    <div className="flex items-center justify-between mb-2">
      <h2 className="font-medium text-gray-800">🧾 Summary</h2>
    ...
    <p className="text-gray-700 text-sm whitespace-pre-wrap leading-relaxed">
      {detail.summary}
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <button
            onClick={() => router.push(`/patients/${id}/calls`)}
