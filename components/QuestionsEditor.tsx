...
interface QuestionsEditorProps {
  patientId?: string | number; // optional during creation
  patientName: string;
...
export default function QuestionsEditor({
  ...
  const addQuestion = () => {
    if (questions.length >= 5) {
      toast.error('⚠️ Maximum 5 questions allowed');
      return;
    }
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-2xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto transition-all animate-fadeIn"
      ...
      >
        ...
        <p className="text-sm text-gray-600 mb-4">
          These questions will be asked during the AI call — one by one.
